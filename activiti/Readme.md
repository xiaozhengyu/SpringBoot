...
1. 排他网关
    ...
    当流程执行到排他网关，会计算所有分支的判断条件，如果结果为true则执行该分支。**注意：**如果存在多个结果为true的分支，则执行id值最小的分支。

    

    **为什么使用排他网关？相比流程变量，排他网关有什么优势？**

    使用流程变量有一个缺点，如果所有分支的条件都不满足，则直接结束流程，并抛出异常。如果排他网关遇到相同的情况，也会抛出异常，但不会结束流程。
